...
void GLCD_clear_xy(unsigned char x, unsigned char y);//부분삭제
...
byte font[95][5] = {            /* 5x7 ASCII character font */
	...
	// E  핀 PE5 [L:비동작, H:동작]  PIN1			00010000 0x10
	// CS1핀 PE7 [L:비동작, H:동작]  PIN16			10010000 0x
	...
	void cmndl( byte cmd )	// left 128x64
	{
		...
		SetBit(PORTE,PORTE7);	// PORTE.7 = 1;		//왼쪽에 데이터 입력
	...
	{
		byte i, j, x, y;
		x = 0xB8;                         /* X start address10111000 */
		y = 0x40;                         /* Y start address01000000 */
		for(i = 0; i <= 7; i++)
		{
			cmnda(x);    cmnda(y);
			for(j = 0; j <= 63; j++)
	...
	void GLCD_clear_xy(unsigned char x, unsigned char y)		//x,y를 클리어한다.
	{
